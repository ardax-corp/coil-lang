...
/// 2 — Direct CALL/RETURN two-slot ABI: CALL bit 31 and RETURN operand `2`
///     move payload+tag. Bit/operand clear still means one word.
///
...
mod tests {
    ...
    fn archive_version_matches_current_abi() {
        ...
        assert_eq!(ARCHIVE_VERSION, pack_archive_version(4, 2));
        assert_eq!(format_archive_version(ARCHIVE_VERSION), "4.2");
    }
