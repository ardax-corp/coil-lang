...
Implicit imports (no `use` needed): prelude (`prelude`, `prelude::ops`, `prelude::test`, `prelude::math`) — injected by the compiler, not written in source. `prelude::math` is IEEE HostInvoke: frozen `sin`/`cos`/`tan`/`sqrt`/`floor`/`ceil`/`exp`/`ln`/`pow` (**102–110**) plus M1 `atan`/`atan2`/`asin`/`acos`, `log10`/`log2`/`cbrt`, `rem`, `sinh`/`cosh`/`tanh` (**125–135**, archive minor 5). Named `PI` / `E` / `TAU` are [coil-stdlib `num`](https://github.com/ardax-corp/coil-stdlib/blob/main/docs/modules.md) (`static const`), not natives.
...
| Control | `if`/`else`, `if let P = e`, `while`, `while let P = e`, `for x in` / `for (k, v) in`, `break`/`continue` |
...
2. **Assuming stdlib** — no `sort` or HTTP in the VM; add [coil-stdlib](https://github.com/ardax-corp/coil-stdlib) ([modules](https://github.com/ardax-corp/coil-stdlib/blob/main/docs/modules.md)) for collections/IO adapters/`num` constants; HTTP is [coil-http](https://github.com/ardax-corp/coil-http) via spool. IEEE `sqrt`/`atan`/… are prelude math; `PI`/`E`/`TAU` are `num`.
