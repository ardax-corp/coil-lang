...
///
/// `coil test` has no thread cases (same constraint as TLS loopback pins in
/// this cargo harness), so these pins live here next to other spawn projects.
...
"#,
    ...
    let output = with_project_cwd(&root, || {
        let mut pipeline = Pipeline::new();
        pipeline.grant_dload_file("sum".to_string(), libsum);
        let (bytecode, constants) = match pipeline.compile_src_from_file(entry.to_str().unwrap()) {
            Ok(pair) => pair,
            Err(()) => {
                for msg in pipeline.messages() {
