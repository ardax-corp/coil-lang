...
fn two_slot_call_followed_by_tag_jump(bc: &[Byte]) -> bool {
    use common::Instruction;
    bc.windows(2).any(|w| {
...
fn method_call_target_relocated_after_static_init_splice() {
    ...
    assert!(
        bytecode
            .iter()
            .any(|b| { matches!(b.bytecode(), Instruction::CALL) && b.call_parts().1 == bump_off }),
...
fn repeated_field_keys_materialize_once_per_function() {
    ...
    let region_end = bc[first_load..]
        .iter()
        .position(|b| matches!(b.bytecode(), Instruction::RETURN | Instruction::BinReturn))
        .map(|i| first_load + i)
        .unwrap_or(bc.len() - 1);
...
fn string_addition_emits_format_not_add() {
    ...
    let via_const = bc.iter().any(|b| {
        matches!(
            b.bytecode(),
            Instruction::CONST | Instruction::ConstReturnImm
        )
    });
    assert!(
...
fn fib_compiles_with_fused_superinstructions() {
    ...
    assert!(
        bc.iter().any(|b| *b.bytecode() == Instruction::BinReturn
...
fn match_jmp_to_end_placeholders_are_patched_to_end_label() {
 ...
 Choice::Maybe(w) => w, \
    ...
    assert!(!bc.is_empty(), "3-arm match must lower to bytecode");
    assert!(
        bc.iter().any(|b| matches!(
            b.bytecode(),
            Instruction::JMP
...
fn const_if_strict_lt_does_not_take_then_branch() {
    use common::Instruction;
    let (bc, _pool) = compile_src("fn main() { if 5 < 5 { return 1; } else { return 0; } }");
    assert!(
...
fn compile_util_then_entry(
    util: &str,
    entry: &str,
    tweak: impl FnOnce(&mut Compiler),
) -> Vec<Byte> {
...
fn cross_module_does_not_inline_private_method() {
    ...
    assert!(!crate::codegen::inline_cost::should_inline_function(
        3, &call, &opts
    ));
...
fn is_tiny_inline_il_accepts_sole_const_return_imm() {
    ...
    let expanded = Compiler::expand_fused_return_for_inline(&ops[0].as_plain_byte().unwrap(), &[])
        .expect("expand");
...
"#;
    ...
    assert!(
        !vm.panicked(),
        "pack(3)==3 && pack(6)==15; opcodes={names:?}"
    );
}
...
"#;
    ...
    let makes = fill_bc
        ...
        .filter(|b| matches!(b.bytecode(), Instruction::MakeArray))
        .count();
    assert_eq!(makes, 1, "one box at return; opcodes={names:?}");
    assert!(
        fill_bc
            .iter()
            .all(|b| *b.bytecode() != Instruction::StoreIndex
...
"#;
    ...
    let makes = pack_bc
        ...
        .filter(|b| matches!(b.bytecode(), Instruction::MakeArray))
        .count();
    assert!(makes >= 1, "box xs at field store; opcodes={names:?}");
    let mut vm = machine::Machine::<64>::with_operand_capacity(64);
    pipeline.wire_host_natives(&mut vm);
    vm.run_raw(
        &bc,
...
"#;
    ...
    assert!(
        pack_bc
            .iter()
            .any(|b| matches!(b.bytecode(), Instruction::HostInvoke | Instruction::CALL)),
...
"#;
    let mut pipeline = crate::Pipeline::new();
    let (bc, constants) = pipeline.compile_src(src).expect("compile");
    let bump_off = pipeline.compiler_mut().get_function("bump").expect("bump");
...
"#;
    ...
    let inits = fill_bc
        ...
        .count();
    assert_eq!(inits, 1, "return identity boxes once; opcodes={names:?}");
    assert!(
...
"#;
    ...
    let inits = hot_bc
        ...
        .count();
    assert_eq!(inits, 1, "call-arg boxes once; opcodes={names:?}");
    assert!(
...
"#,
    ...
    assert!(
        bc.iter()
            .any(|b| matches!(b.bytecode(), Instruction::CONST)),
...
fn concrete_add_still_emits_add() {
    use common::Instruction;
    let (bc, _pool) = compile_src(
        "fn add(int a, int b) -> int { return a + b; } fn main() { let x = 0; while x < 1 { x = x + 1; } return add(x, 2); }",
    );
    assert!(
        !bc.iter()
            .any(|b| matches!(b.bytecode(), Instruction::DynAdd)),
...
fn generic_call_with_concrete_arg_emits_box_value() {
    use common::Instruction;
    let (bc, _pool) =
        compile_src("fn id<T>(T x) -> T { return x; } fn main() { let f = id; f(42); }");
    assert!(
        bc.iter()
...
"#,
    ...
    let make_array = bc
        .iter()
        .find(|b| matches!(b.bytecode(), Instruction::MakeArray) && b.operand_u32() == 3)
...
"#,
    ...
    assert!(
        ...
        "heap Vec::pop must ask the host for OptionNiche; operands={:?}",
        hosts
            .iter()
            .map(|b| format!("{:#x}", b.operand_u32()))
            .collect::<Vec<_>>(),
    );
}
...
"#,
    ...
    assert!(
        bc.iter()
            .filter(|b| matches!(b.bytecode(), Instruction::HostInvoke))
            .all(|b| {
                common::host_invoke_enum_layout(b.operand_u32()) == common::HOST_ENUM_LAYOUT_BOXED
            }),
        "int Vec::pop stays boxed at the host edge; opcodes={:?}",
        bc.iter().map(|b| b.bytecode()).collect::<Vec<_>>(),
    );
}

#[test]
fn gc_get_heap_item_sets_option_niche_layout() {
    let mut ast = Pratt::default()
        .parse(
            r#"
use gc::{get, root};
...
"#,
        ...
        .expect("parse");
    let mut compiler = Compiler::default();
    compiler.register_native_id("gc_root", 1);
    compiler.register_native_id("gc_get", 2);
    let bc = compiler.compile("", &mut ast);
    assert!(
        bc.iter().any(|b| {
        ...
        "gc::get of a heap object must set OptionNiche; opcodes={:?}",
        bc.iter()
...
"#,
    ...
    assert!(
        ...
        "two-slot `?` uses tag JMP, not JumpIfMatch / pair opcodes; opcodes={:?}",
        bc.iter().map(|b| b.bytecode()).collect::<Vec<_>>(),
    );
    assert!(
        two_slot_call_followed_by_tag_jump(&bc),
        "two-slot `?` should branch on the tag, not DUP+EQ; opcodes={:?}",
        bc.iter().map(|b| b.bytecode()).collect::<Vec<_>>(),
    );
}
...
"#,
    ...
    // Identity `return e?` / `return Ok(e?)` is CALL then RETURN, no tag test.
    let has_call_then_return = bc.windows(2).any(|w| {
        matches!(w[0].bytecode(), Instruction::CALL)
            && w[0].call_ret_words() == 2
            && matches!(w[1].bytecode(), Instruction::RETURN)
            && w[1].return_words() == 2
    });
    assert!(
        has_call_then_return,
        "return e? should forward the pair; opcodes={:?}",
        bc.iter()
            .map(|b| format!("{:?} {:#x}", b.bytecode(), b.operand_u32()))
            .collect::<Vec<_>>(),
    );
...
fn result_try_churn_does_not_make_enum() {
    ...
    assert!(
        two_slot_call_followed_by_tag_jump(&bc),
        "result_try_churn `?` should branch on the tag; opcodes={:?}",
        bc.iter().map(|b| b.bytecode()).collect::<Vec<_>>(),
    );
}
