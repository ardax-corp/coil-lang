...
impl Compiler {
    ...
    fn compile_construct_expr<'compiler>(
            ...
            if self.functions.contains_key(&fqn) || self.fn_entry_labels.contains_key(&fqn) {
                let arg_slice: &[Output] = match fields {
                    ...
                    EnumConstructPayload::Record(parts) => {
                        ...
                        let _ = self.emit_direct_fn_call(&mut bytecode, &fqn, parts.len() as u32);
                        return bytecode;
                    }
                };
        ...
        if self.should_niche_unit_result_construct(enum_name, ast) {
            match (variant_name, fields) {
    ...
    fn try_emit_folded_expr(
        ...
        if let Some(v) = crate::const_fold::eval_expr(ast, self.const_env()) {
            ...
            true
        } else if let Some(bit) = crate::const_fold::strength_reduce_bitops(ast, self.const_env()) {
            match bit {
            ...
            && let Some((inner, shift)) =
                crate::const_fold::strength_mul_to_shl(ast, self.const_env())
        {
                ...
                crate::const_fold::strength_div_to_shr(ast, self.const_env())
        {
            ...
            crate::const_fold::strength_pow_int(ast, self.const_env())
        {
            use crate::typechecking::subst::apply_ty_prune;
            use crate::typechecking::ty::{BYTE, INT};
    ...
    fn resolve_tail_callee_key(&self, expr: &Output<'_>) -> Option<String> {
        ...
        if let Some((fa, is_rest, id)) =
            self.sidecar_overload(None, call_expr.0.start, call_expr.0.end)
        {
    ...
    fn return_is_tail_match(&self, expr: &Output<'_>) -> bool {
        ...
        !arms.is_empty()
            && arms
                .iter()
                .all(|arm| self.expr_is_tail_self_call(&arm.body))
    }
    ...
    fn try_inline_direct_call_into(
        ...
        for byte in &slice {
            ...
            } else if matches!(byte.bytecode(), Instruction::STORE | Instruction::StorePop) {
    ...
    fn try_remat_peel_call_into(
        ...
        for arg in &flat {
            ...
            if split || plain.as_ref().is_none_or(|p| !Self::peel_remat_bytes_ok(p)) {
                ...
                argv.push(vec![Byte::new(Instruction::LOAD).with_load_store_slot(tmp)]);
            } else {
        ...
        self.bytecode
            .push(Byte::new(Instruction::CALL).with_call_packed(argv.len() as u32, target_offset));
        self.bytecode.bind_label(join);
        let result = self.alloc_temp_slot();
        self.bytecode.push_store_pop(result);
        bytecode.push_load(result);
    ...
    fn peel_remat_op(op: &IlOp, argc: usize, args: &mut Vec<usize>) -> Option<PeelRematOp> {
        ...
        match op {
            ...
            | IlOp::ConstPool { .. }
            | IlOp::String { .. }
            | IlOp::Dup { .. } => Some(PeelRematOp::Copy(op.clone())),
...
impl Compiler {
    ...
    fn push_match_bindings(&mut self, inner: HashMap<String, u32>) -> Option<HashMap<String, u32>> {
        ...
        match node.1.as_ref() {
            ...
            Expression::Call { name, args } => {
                self.expr_mentions_stack_array(name)
                    || args.as_ref().is_some_and(|items| {
                        items.iter().any(|a| self.expr_mentions_stack_array(a))
                    })
            }
    ...
    fn alloc_stack_array_slots(&mut self, name: &str, n: usize) -> u32 {
        ...
        for i in 1..n {
            ...
            debug_assert_eq!(
                slot,
                base + i as u32,
                "stack array slots must be consecutive"
            );
        }
        self.context
            .stack_array_locals
    ...
    fn emit_escape_stack_array(&mut self, bytecode: &mut CodeBuf, name: &str, base: u32, n: usize) {
    ...
    fn emit_call_args_with_rest(
        ...
        if !pack_rest && fixed.iter().any(|a| self.expr_may_clobber_operand_stack(a)) {
            let n = self.emit_call_args_stage_all(&fixed, bytecode, box_generic);
            self.park_args_above_stack_array_boxes(bytecode, n);
            return n;
        }
        ...
        match expr.1.as_ref() {
            ...
            Expression::Construct { fields, .. } => {
                ...
                match fields {
                    ...
                    EnumConstructPayload::Tuple(args) => {
                        args.iter().any(|a| self.expr_may_clobber_operand_stack(a))
                    }
    ...
    fn expr_is_stackable_direct_call(&self, expr: &Output<'_>) -> bool {
        ...
        if !self.functions.contains_key(&fqn)
            && !self.functions.contains_key(strip_overload_key(&fqn))
        {
    ...
    fn stack_array_index_proven(
        ...
        self.node_in_bounds_index(index_node) || Self::stack_array_mod_index_proven(index, n)
    }
    ...
    fn unboxed_class_field_slot(&self, receiver: &Output<'_>, field: &str) -> Option<u32> {
    ...
    fn alloc_unboxed_class_slots(&mut self, name: &str, class_name: &str, n: usize) -> u32 {
        ...
        self.context
            .unboxed_class_locals
            .insert(name.to_string(), (base, n, class_name.to_string()));
        base
    }
    ...
    fn ty_is_unbox_local(&self, ty: &Ty) -> bool {
        if self.checker.ty_is_class(ty) {
            ...
            let n = self
                ...
                .map(|f| f.len())
                .unwrap_or(0);
            return crate::escape::ClassEscape::for_named_new(self.checker.class_has_drop(name), n)
                .stack_allocatable();
        }
    ...
    fn try_emit_aggregate_arith(&mut self, args: TryEmitAggregateArithArgs<'_, '_>) -> bool {
        ...
        let info = self_id
            ...
            .or_else(|| {
                self.checker
                    .aggregate_arith_span(span_start, span_end)
                    .cloned()
            })
    ...
    fn emit_ffi_invoke(&mut self, span: SimpleSpan, args: &[Output]) {
        ...
        if variadic {
            ...
            if let Some(tags) = self.resolve_call_ffi_tags(None, (span.start, span.end), &args) {
                for &(tag, aux) in &tags {
                    emit_ffi_type_const(&mut self.bytecode, tag, aux);
                }
        ...
        match ty.map(|ty| self.value_layout(&ty)) {
            Some(ValueLayout::NicheUnitResult) => {
                Self::emit_boxed_result_to_niche(&mut self.bytecode, true);
            }
            Some(ValueLayout::NicheResult) => {
    ...
    fn emit_show_for_format_arg(&mut self, arg: &Output) {
        ...
        if let Some(ty) = arg_ty.as_ref() {
                ...
                && (self.functions.contains_key(&fqn) || self.fn_entry_labels.contains_key(&fqn))
            {
                let mut arg_bc = self.do_compile(arg);
                self.bytecode.append(&mut arg_bc);
                if self.expr_layout(arg).is_niche_option() {
    ...
    fn emit_prelude_host_call(
        ...
        self.emit_host_native_invoke(native_name, args, result);
    }
    ...
    fn can_flatten_try_pair(&self, inner: &Output, enum_name: &str) -> bool {
        ...
        if self.expr_layout(inner).is_niche_option()
            || self.expr_layout(inner).is_niche_unit_result()
    ...
    fn emit_try_pair_failure(&mut self, inner_kind: &str) {
        if let Some(outer) = self.compiling_two_word_enum.clone() {
            ...
            self.push_return_two_word();
        } else if self.return_layout().is_niche_result() {
            ...
            self.bytecode.push_return();
        } else if self.return_layout().is_niche_unit_result() {
            self.bytecode.push_pop();
            self.bytecode.push_return();
        } else {
            let mut boxed = std::mem::take(&mut self.bytecode);
    ...
    fn index_keeps_array_on_stack_safe(idx: &Expression<'_>) -> bool {
        matches!(idx, Expression::Identifier(_) | Expression::Integer(_))
    }
    ...
    fn emit_for_in_array_loop(
        ...
        if !array_already_on_stack {
            let mut iter_bc =
                self.do_compile(iterable.expect("iterable required when not on stack"));
            ...
            && let Expression::Range { start, end, .. } = iterable.1.as_ref()
                ...
                && let Some(trips) = crate::const_fold::range_trip_count(start, end, inclusive) {
                    ...
                    if let Some(ConstValue::Int(s)) =
                        crate::const_fold::eval_expr(start, self.const_env())
                    {
                        for k in 0..trips {
                            let val = s + k as i64;
    ...
    fn emit_for_in_range_latch(
        ...
        let alias_iv =
            bind.pattern.is_none() && !crate::const_fold::body_assigns_ident(body, bind.name);
        let binding_slot = self.alloc_binding_slot(bind.name);
        if alias_iv {
    ...
    fn emit_for_in_custom(&mut self, args: EmitForInCustomArgs<'_, '_>) {
        ...
        match counted {
            ...
            Some(ForInCounted::Range { inclusive, float }) => {
                let cur_slot = self.alloc_temp_slot();
                let end_slot = self.alloc_temp_slot();
                self.bytecode.push_store_pop(end_slot);
                self.bytecode.push_store_pop(cur_slot);
        ...
        let none_tag = self
            ...
            .unwrap_or(0);
        let niche_next = item_ty.is_some_and(|ty| crate::typechecking::value_layout::niche_heap_only(&self.checker, ty));
        ...
        if let Some(item) = item_ty {
            ...
            let kind =
                crate::typechecking::return_layout::two_word_return_enum(&self.checker, &opt);
        ...
        if niche_next {
            ...
            bb.emit_jump_to(exit_label, BbJumpKind::JumpIfTrue, self.bytecode.il_mut());
    ...
    fn is_result_construct(expr: &Output) -> bool {
        match expr.1.as_ref() {
            Expression::Construct { enum_name, .. } => common::is_builtin_result_enum(enum_name),
    ...
    fn wrap_result_err_on_stack(&mut self) {
        if self.return_layout().is_niche_unit_result() {
            return;
        }
        if self.return_layout().is_niche_result() {
            Self::push_result_err_bit(&mut self.bytecode);
        } else {
    ...
    fn try_emit_matrix_op(
        ...
        let Some(info) = self_id
            ...
            .or_else(|| {
                self.checker
                    .linear_algebra_span(span_start, span_end)
                    .cloned()
            })
        else {
            return false;
        ...
        match child.borrow() {
                ...
                if children.len() == 2 {
                    ...
                    if let Some((name, is_const)) = binding {
                        ...
                        if self.try_bind_frame_local(
                            ...
                            if let Some(val) =
                                crate::const_fold::eval_expr(&children[1], self.const_env())
                            {
                            ...
                            if let Some(n) = stack_n {
                                let can_stack = match rhs_node.1.as_ref() {
                                    ...
                                    Expression::Identifier(src) => {
                                        self.stack_array_info(src).is_some_and(|(_, sn)| sn == n)
                                    }
                                    _ => false,
                                };
                                if can_stack {
            ...
            Expression::Index(target, Some(index)) => {
                    ...
                    && (1..=32).contains(&n)
                {
                    ...
                    let idx_slot = self.alloc_temp_slot();
                    bytecode.push_store_pop(idx_slot);
                    self.emit_stack_array_select_load(&mut bytecode, base, n, idx_slot, proven);
                } else if let Expression::Identifier(name) = target.1.as_ref()
            ...
            Expression::Return(expr) | Expression::ImplicitReturn(expr) => {
                ...
                if self.compiling_two_word_enum.is_none() && self.compiling_result_mode {
                    ...
                    if !self.skip_result_ok_wrap_for_return(expr)
                        && !self.return_layout().is_niche_result() && !self.return_layout().is_niche_unit_result() {
                ...
                if identifier.is_some() || pattern.is_some() {
                    ...
                    match kind {
                        ForInKind::Array => {
                            ...
                            self.emit_for_in_array_loop(body, &bind, false, Some(iterable), pin);
                        }
            ...
            Expression::Try(inner) => {
                ...
                if self.expr_layout(inner).is_niche_option() {
                    ...
                    bb.bind_label(success, self.bytecode.il_mut());
                } else if self.expr_layout(inner).is_niche_unit_result() {
                    Self::push_niche_eq_zero(&mut self.bytecode);
                    bb.emit_jump_to_hinted(
                        success,
                        BbJumpKind::JumpIfTrue,
                    ...
                    bb.bind_label(success, self.bytecode.il_mut());
                } else if self.expr_layout(inner).is_niche_result() {
            ...
            Expression::Coalesce(lhs, rhs) => {
                ...
                let niche_lhs = self.expr_layout(lhs).is_niche_option();
                let unit_result_lhs = self.expr_layout(lhs).is_niche_unit_result();
                let niche_result_lhs = self.expr_layout(lhs).is_niche_result();
            ...
            Expression::OptionalAccess(receiver, field) => {
                ...
                if self.expr_layout(receiver).is_niche_option()
                    && self.expr_layout(ast).is_niche_option()
                {
                    ...
                    let end = bb.fresh_label(self.bytecode.il_mut());
                    Self::push_niche_eq_zero(&mut self.bytecode);
                    bb.emit_jump_to(end, BbJumpKind::JumpIfTrue, self.bytecode.il_mut());
