use super::*;
use crate::typechecking::env::{TyVarCounter, instantiate};
...
fn expr_value_ty(c: &Checker, expr: &Output) -> Option<Ty> {
    ...
    match expr.1.as_ref() {
        Expression::Identifier(name) => {
            ...
            Some(apply_ty_prune(
                c.subst(),
                &instantiate(scheme, &mut counter),
            ))
        }
...
fn if_single_branch() {
    ...
    let mut counter = TyVarCounter::new();
    let ty = peel_fn_return(apply_ty_prune(
        c.subst(),
        &instantiate(scheme, &mut counter),
    ));
    assert_eq!(ty, int());
...
fn class_visibility_recorded_for_future_member_access() {
    ...
    assert!(Checker::can_access_member(Visibility::Public, "Foo", None));
...
fn builtin_len_unknown_named_arg_errors() {
    ...
    assert!(
        msgs.iter()
            .any(|m| m.message().contains("Unknown named argument")),
        ...
        msgs.iter().any(|m| {
            m.message().contains("payload shape mismatch")
                && m.help().as_ref().is_some_and(|h| {
                    h.contains("wrapping") && h.contains("JsonObject") && h.contains("Obj(value)")
                })
        }),
...
fn access_field_from_tuple_variant_produces_error() {
    ...
    let diag = msgs
        .iter()
        .find(|m| {
            m.message().contains("Cannot access field") || m.message().contains("has no field")
        })
        .unwrap_or_else(|| panic!("expected field-access diagnostic, got: {:?}", msgs));
...
fn len_rejects_non_array() {
    ...
    let found = msgs
        .iter()
        .any(|m| m.message().contains("No instance for `Length") || m.message().contains("Length"));
    assert!(
        found,
...
fn dict_duplicate_field_emits_diagnostic() {
    ...
    let found = msgs.iter().any(|m| {
        m.code() == Some(ErrorCode::DuplicateField) && m.message().contains("Duplicate field")
    });
    assert!(
        found,
...
fn let_record_duplicate_field_emits_diagnostic_if_parse_bypassed() {
    ...
    let found = msgs.iter().any(|m| {
        m.code() == Some(ErrorCode::DuplicateField)
            && m.message()
                .contains("Duplicate field `x` in record pattern")
    });
    assert!(
        found,
        "expected duplicate-field diagnostic, got: {:?}",
...
fn record_construct_duplicate_field_emits_diagnostic_if_parse_bypassed() {
    ...
    let found = msgs.iter().any(|m| {
        m.code() == Some(ErrorCode::DuplicateField)
            && m.message()
                .contains("Duplicate field `x` in record constructor")
    });
    assert!(
        found,
        "expected duplicate-field diagnostic, got: {:?}",
...
"#;
    ...
    assert!(
        msgs.iter()
            .any(|m| m.message().contains("No instance for `Num")),
...
"#;
    ...
    assert!(
        msgs.iter()
            .any(|m| m.message().contains("Cannot find function")),
    ...
    assert!(
        msgs.iter().any(|m| {
            m.code() == Some(ErrorCode::InvalidTry)
                ...
                && m.help().as_ref().is_some_and(|h| h.contains("raise err;"))
        }),
...
"#;
    ...
    assert!(
        msgs.iter().all(
            |m| !m.message().contains("Cannot satisfy") && !m.message().contains("No instance")
        ),
...
fn string_var_annotated_vec_byte_rejects() {
    ...
    assert!(
        msgs.iter().any(|m| {
            let msg = m.message();
            msg.contains("Type mismatch") || msg.contains("expected") || msg.contains("Vec<byte>")
        }),
        "got: {:?}",
        msgs.iter().map(|m| m.message()).collect::<Vec<_>>()
    );
...
fn scalar_enum_does_not_accept_raw_int() {
    ...
    assert!(
        msgs.iter()
            .any(|m| m.message().contains("int") || m.message().contains("Status")),
