...
#[test]
fn dest_prop_keeps_alias_after_getfield_unlike_slot_promote() {
    let mut ops = vec![
        load(0),
        store(2),
        load(2),
    ...
    dest_prop(&mut ops, 3);
    assert!(
        matches!(ops[5], IlOp::Load { slot: 0, .. }),
        "second GetField must still see src; slot_promote clears at GetField"
    );
}
