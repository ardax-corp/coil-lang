...
|-------|------------------------|------------------|
| Q6 | Counted desugar for array / Vec / `[T; N]` / literal range (`for_in_sum` / `for_in_range`). First-class `let r = 0..n` locals unbox (B5 / `for_in_range_value`). User `Iterator` / coro / dict / parameter range still fuse-IL | Phased ladder. Not a permanent fuse-IL ceiling. See [q6-iterator-protocol.md](q6-iterator-protocol.md). |
