...
|-------|------------------------|------------------|
| Q6 | Counted desugar for array / Vec / `[T; N]` / literal range (`for_in_sum` / `for_in_range`). First-class `let r = 0..n` locals unbox (B5 / `for_in_range_value`). Free-fn param / returned numeric Range unbox (C2 / `for_in_range_param` / `for_in_range_ret`). Heap-field / array-held Range counted (C2b rung 1 / `for_in_range_field` / `for_in_range_array`). User `Iterator::next` on MIR (C2b rung 2 / `for_in_iter`); counted `into_iter` Range (`for_in_iter_range`). Dict for-in on MIR (C2b rung 3 / `for_in_dict` / `for_in_dict_pairs`; `DictEntries` + counted latch; `for (k, v)` is bind sugar). Coro for-in on MIR (C2b rung 4 / `for_in_coro`; `ResumeCoro` / `DoneCoro`) | Phased ladder. Not a permanent fuse-IL ceiling. See [q6-iterator-protocol.md](q6-iterator-protocol.md). |
| Q7 | One-word self-`CALL` / `TailCall` may enter dense | Landed as a cost-gated path (not a hard wall). **B2** convoys tight `tak` / `fib` on the operand stack so they can win that gate. **B7** sibling / mutual `TailCall` may dense (stack-arg + reserved callee entries). **C1** self two-slot `CALL` / `RETURN` may dense or LIR. LIR one-word `CALL` still refuse. |
...
| Q9 | **R1–R3 landed:** table `STRING` / `PRINT` / `FORMAT` / `STRINGIFY` are SSA + MIR→LIR (dense infer still refuses). `from_bytes` / `to_bytes` are I6 dense HostInvoke. `FORMAT` / `STRINGIFY` take I5-style maps. Unicode / regex later (R4) | Phased ladder. Full format / string on MIR. No half-format second lowering. See [q9-format-string.md](q9-format-string.md). |
