...
|-------|------------------------|------------------|
| Q6 | Counted desugar for array / Vec / `[T; N]` / literal range (`for_in_sum` / `for_in_range`). First-class `let r = 0..n` locals unbox (B5 / `for_in_range_value`). Free-fn param / returned numeric Range unbox (C2 / `for_in_range_param` / `for_in_range_ret`). Heap-field / array-held Range counted (C2b rung 1 / `for_in_range_field` / `for_in_range_array`). User `Iterator::next` on MIR (C2b rung 2 / `for_in_iter`); counted `into_iter` Range (`for_in_iter_range`). Dict for-in on MIR (C2b rung 3 / `for_in_dict`; `DictEntries` + counted latch). Coro for-in on MIR (C2b rung 4 / `for_in_coro`; `ResumeCoro` / `DoneCoro`) | Phased ladder. Not a permanent fuse-IL ceiling. See [q6-iterator-protocol.md](q6-iterator-protocol.md). |
