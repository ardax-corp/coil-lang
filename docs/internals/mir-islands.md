...
| Heap index / `MakeArray` / alloc | SSA `Alloc` + `GcBarrier` when `allow_alloc`; S2a live-heap `roots`; S2b frame maps; S2c dense / LIR when maps exist; S2d mapped in-loop / preheader Make*. S3b emits unpinned `Index` / `StoreIndex` / `ArrayLen` + `Seek` restore (pins unsound next to dense `Seek`). In-loop `Vec.push` / class `new` stay fuse-IL. Dense+match stays LIR (I2) | **I5** / **S2a** / **S2b** / **S2c** / **S2d** / **S3** / **S3b** |
