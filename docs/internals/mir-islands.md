...
| `for` / iterators | **Q6 counted desugar** on array / Vec / `[T; N]` / literal range helpers (`for_in_sum` `sum`, `for_in_range`); **B5** first-class `let r = 0..n` locals (`for_in_range_value`); **C2** free-fn param / returned numeric Range (`for_in_range_param` / `for_in_range_ret`); **C2b rung 1** heap-field / array-held Range (`for_in_range_field` / `for_in_range_array`); **C2b rung 2** user `Iterator::next` (`for_in_iter`) and counted `into_iter` Range (`for_in_iter_range`); **C2b rung 3** dict for-in (`for_in_dict` / `for_in_dict_pairs`, `DictEntries` + counted latch); **C2b rung 4** coro for-in (`for_in_coro`, `ResumeCoro` / `DoneCoro`); `main` + format stays fuse-IL | phased ladder — [q6-iterator-protocol.md](q6-iterator-protocol.md); not a permanent fuse-IL ceiling |
