...
| **`escape_analysis`** | Immediate-only `MakeArray` (arity ≤ 32) → consecutive frame slots; S2g boxes at named escape edges | Growing `ArrayPush` dest, private-after-escape, computed elements stay heap. Unproven `xs[k]`: leftover MakeArray stays heap; codegen `[T;N]` uses S2h OOB-safe select. **Not** named-local class SROA (COI-84). |
