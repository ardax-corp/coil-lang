...
| **`escape_analysis`** | Shared Q1 answer (`escape::ArrayEscape`): non-escaping `MakeArray` (arity ≤ 32) → slots; named escape → **box once** | Grow dest / private-after-escape stay heap (**Q3**: `[T; N]` grow is a type error). Computed elems use the same rule (S2i). Unproven `xs[k]`: leftover MakeArray stays heap; codegen `[T;N]` uses S2h OOB-safe select + Q4 Euclidean `i % N`. Named class SROA is codegen / `local_escape` (S2j / **Q2**). |
