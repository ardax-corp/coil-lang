...
| **`escape_analysis`** | Immediate-only `MakeArray` (arity ≤ 32) → consecutive frame slots; S2g boxes at named escape edges | Growing `ArrayPush` dest, private-after-escape, computed elements, unproven `xs[k]` stay heap. **Not** named-local class SROA (COI-84). |
