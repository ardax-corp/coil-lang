...
6. **Execute** — the VM in `machine/` loads the archive and runs `main` (or each `test("…")` case under `coil test`). `thread::spawn` workers copy the parent `[ffi] search_paths`, `dload` gate, and entry `base_dir`; they do not inject a cwd `./native/libtls` fallback (COI-233).
