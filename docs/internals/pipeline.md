...
5. **Archive** — `coil compile` wraps bytecode in a versioned `ArchivedProgram` envelope (packed `major.minor` in `common/src/archive.rs`) and writes `out.hyc` (or `-o`). Default `coil file.hy` compiles and runs **in memory** and does not read or write `out.hyc`. See [Debug line table](debug-info.md). **Policy:** `.hyc` does not record `[env]` exec/exit/FFI-exec grants (no rkyv major for grants). `coil run` / `coil-embed` stay deny-all for those operations unless the dload trailer already encodes attach. In-memory compile+run still applies `coil.toml` `[env]` on the `Machine` via `wire_vm_host`.
