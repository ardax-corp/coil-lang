...
3. **Codegen (stack IL)** — instruction lowering, not a semantic IR: `compiler/src/lib.rs` emits a compile-time stack IL (`compiler/src/il/`) with **symbolic labels** for jumps/joins (`IlOp::Jump` / `IlOp::Label`, not absolute PCs). Semantic structure is DefIds / typed sidecar. Ground heap-only `Option<T>` values use a pointer niche; statically known unary `Option`/`Result` returns can use a two-slot pair until a boxed boundary.
