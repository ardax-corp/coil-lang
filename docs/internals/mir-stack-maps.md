...
- Dense specialize / MIR→LIR **may cross alloc** when
  ...
  **in-loop** `Make*` dense after SROA/LICM and keeps it only when emit
  cost ≤ fuse-IL (LOAD/STORE boxing still loses; A/B:
  [s2d-inloop-make-tax.md](s2d-inloop-make-tax.md)). `COIL_S2D_DENSE_INLOOP=1`
  forces dense; `=0` restores the S2e refuse. Compare-only leftovers
...
4. ~~**Looping alloc**~~ — **S2d / S2e.** Mapped preheader `Make*` + index
   ...
   fuse-IL unless SROA/LICM deletes it or the S2l cost-gate says the
   reconstruct is ≤ fuse-IL. Still refuse: post-loop-only heap return
