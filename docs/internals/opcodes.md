...
| `DenseBinJmp` | Two-word pack of `DenseBin` then `JMP` (COI-387 X2). Counted-loop latch (nsieve k-loop, SIMD remainder `i += 1`). Operand is the bin `dense_abc`; the following word is the `JMP`. Not S1 (not a cmp) and not S2 (not two bins). Archive **minor 20**. |
| `DenseBin2Jmp` | Three-word pack of `DenseBin2` then `JMP` (COI-387 X2). First `dense_abc` is the opcode operand; payload word is the second `DenseBin`; third word is the `JMP`. Mandelbrot inner latch after S2. Sequential IEEE; not `FloatChainStore`. Archive **minor 20**. |
