...
| `DenseBinJmp` | Two-word pack of `DenseBin` then `JMP` (COI-387 X2). Counted-loop latch (nsieve k-loop, SIMD remainder `i += 1`). Operand is the bin `dense_abc`; the following word is the `JMP`. Not S1 (not a cmp) and not S2 (not two bins). Not ALWAYS_HOT. Archive **minor 20**. |
| `DenseBin2Jmp` | Three-word pack of `DenseBin2` then `JMP` (COI-387 X2). First `dense_abc` is the opcode operand; payload word is the second `DenseBin`; third word is the `JMP`. Mandelbrot inner latch after S2. Sequential IEEE; not `FloatChainStore`. Not ALWAYS_HOT. Archive **minor 20**. |
...
| `DenseIndex` / `DenseStoreIndex` | A2 (archive **minor 10**). Stack-neutral heap index / store. `[31:24]` flags (bit 0 = unchecked), `[23:16]` dest, `[15:8]` array, `[7:0]` index. `DenseStoreIndex` dest is the stored value. Last-addr `Object` cache + `frame_pins` (COI-372); slab probe only on identity miss. |
