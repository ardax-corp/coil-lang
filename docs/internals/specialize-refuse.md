...
| `pack` | `s2d_inloop_escape.hy` | dense-native or fuse-IL | A2: `DenseMake` + cost gate; boxed residual Make* still fuse-IL |
...
SROA/LICM. A2 ([COI-335](https://linear.app/ardax/issue/COI-335)) emits
`DenseIndex` / `DenseMake` / `DensePush` so leftover escaping Make* can
stay dense when the reconstruct is native and cost ≤ fuse-IL. Boxed
LOAD/STORE residuals still lose ([s2d-inloop-make-tax.md](s2d-inloop-make-tax.md)).
...
| `MakeArray` / alloc / GC safepoints | SSA `Alloc` + `GcBarrier`; S2a roots; S2b maps; S2c dense / LIR **only when maps exist**; S2d mapped preheader Make*; S2e Seek-less residuals; S2f SROA / StoreIndex reuse / hoist of non-escaping Make*; S2g box at named escape edges (**Q1**: box-once, not fresh-per-edge); S2h unproven `xs[k]` OOB-safe select / heap pick; S2i observed/escape computed elems stay heap Index/StoreIndex; S2j named class field SROA (non-escaping only, `local_escape`; **Q2**); S2k proven select diamonds dense/LIR when Seek ≤ 64; S2l + **A2** (`DenseMake` / `DenseIndex` / `DensePush`; cost gate refuses boxed denser-but-slower); unmapped fuse-IL; post-loop-only `return [x]` fuse-IL | **I5** / **S2a**–**S2l** / **A2** |
