...
| **S3** | [COI-380](https://linear.app/ardax/issue/COI-380) | `DenseCast`+`DenseBin` | Three `DenseCast` sites, all **x/y headers** (`DenseCast ; DenseBin×3`), not the innermost `iter` loop. | dense-streak VM peek | **Done (peek, no opcode).** After S2 the header is `DenseCast ; DenseBin2 ; DenseBin`. Table/hotmatch consume the trailing `DenseBin`/`DenseBin2` in the `DenseCast` handler; X4 also takes leftover `DenseBin` after that `DenseBin2`. Bytecode unchanged; no archive bump; not ALWAYS_HOT growth. Giant match stays split. Flagship wall is header-noise — prove on `examples/perf/dense_cast_bin.hy`. `fib` has no this shape; keep `unlikely(is_hot)`. |
...
| **X4** | VM coalescing of adjacent `DenseBin` **without** a new opcode | same as S2 | `machine/src/dispatch.rs` dense-streak peek | **Cancel (A/B on main after S2).** Peek leftover `DenseBin` after `DenseBin2` only (odd-length residue / headers). Dissect **does** fire: mandelbrot y/x headers `DenseCast ; DenseBin2 ; leftover DenseBin`; inner latch `DenseBin2 ; leftover IADD ; JMP`. No unpacked `DenseBin; DenseBin` (S2). Fat LTO `COIL_AUTO_PAR=0` hyperfine vs `main` (same `.hyc`): mandelbrot 20.4±2.9 vs 20.7±2.4 ms (**wash**); fib 60.4±1.5 vs 60.9±2.2 (**flat**, no discriminant tax); nsieve 2.3 vs 2.3 (**wash**). `dense_cast_bin` hit ~1.08× then washed on a second run. Do **not** merge for flagship walls. Giant match does not peek. No archive bump. |
...
`DenseBin2` is a two-word ISA pack of adjacent `DenseBin` (COI-381); COI-389 X4
peeks a leftover `DenseBin` after `DenseBin2` in the dense streak only (odd-length
residue; no extra discriminant; giant match does not). COI-387 X2 peeks a trailing `JMP` after
`DenseBin`/`DenseBin2` in the dense streak only (no extra discriminant).
COI-380 S3 peeks a trailing `DenseBin`/`DenseBin2` after `DenseCast` on
the dense streak only (no extra discriminant). COI-382 S5 peeks trailing
`DenseBin`/`DenseBin2` then `JMP` after `DenseStoreIndex` in the dense streak
...
- y/x headers: `DenseCast ; DenseBin2 ; DenseBin ; DenseMove* ; BinSlotSlotJmpf` (the dense streak peeks the bin after `DenseCast`, including X4 leftover `DenseBin` after `DenseBin2`)
...
`DenseMove ; DenseStoreIndex ; DenseBin ; JMP` (the dense streak peeks store + bin + latch JMP).
