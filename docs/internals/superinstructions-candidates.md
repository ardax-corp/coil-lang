...
peeks a leftover `DenseBin` after `DenseBin2` on table/hotmatch only (odd-length
...
- y/x headers: `DenseCast ; DenseBin2 ; DenseBin ; DenseMove* ; BinSlotSlotJmpf` (table/hotmatch peeks the bin after `DenseCast`, including X4 leftover `DenseBin` after `DenseBin2`)
