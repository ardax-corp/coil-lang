...
| **X4** | VM coalescing of adjacent `DenseBin` **without** a new opcode | same as S2 | `machine/src/dispatch.rs` table/hotmatch peek | **A/B on main (S2 already shipped).** Peek leftover `DenseBin` after `DenseBin2` only (odd-length residue). Do **not** peek `DenseBin; DenseBin` on the `DenseBin` arm (S2 packed those; extra work taxes fib). Giant match does not peek (debugger / `debug_locs` stay per-op). No archive bump; keep `unlikely(is_hot)`. Land only if wall beats S2; otherwise cancel. |
