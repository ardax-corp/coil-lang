...
| **S6** | [COI-383](https://linear.app/ardax/issue/COI-383) | `DenseMove` coalesce | `mandelbrot` **9** `DenseMove` (φ / latch copies). Pairs: `DenseMove ; DenseMove ; BinSlotSlotJmpf`. | MIR destprop / dense emit | **Done (peep, no opcode).** Sink φ incomings past last use of the dest, then interference-coalesce φ dest with incomings (latch first). Residual identity / unique-writer `DenseMove` dest-rewrite in `emit.rs` `coalesce_dense_moves`. Must not flip cost-gate keep (LOAD/STORE cost 2 vs `DenseMove` 1). No specialize refuse change. |
...
Kick order (project ladder, still valid): **S8 note first** (this doc) → **S2** (landed, two-word `DenseBin2`) → **S1** (landed, two-word `DenseBinJmpf`) → **S4** (landed, two-word `DenseIndexJmpf`) → **S6** → **S3** (colder) → **S5** (after shape refine) → **S7** (after choosing a real fuse-IL hit).
...
- y/x headers: `DenseCast ; DenseBin×3 ; DenseMove* ; BinSlotSlotJmpf` (S6 coalesces nested `sum` and some inits)
