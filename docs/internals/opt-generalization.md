...
| `for` / iterator (`for_in_sum`) | **Q6:** counted desugar — array / Vec / `[T; N]` / literal range helpers dense (`sum`, `for_in_range`). **B5:** first-class `let r = 0..n` locals unbox `[start,end]` (`for_in_range_value`). **C2:** free-fn param / returned numeric Range two-slot (`for_in_range_param` / `for_in_range_ret`). **C2b rung 1:** heap-field / array-held Range counted (`for_in_range_field` / `for_in_range_array`). **C2b rung 2:** user `Iterator::next` (`for_in_iter`) and counted `into_iter` Range (`for_in_iter_range`). `main` + format / `Vec.push` stays fuse-IL. Dict / coro for-in still refuse | Ladder (counted + local + param/ret + heap-field range + user `next`) |
...
| **C2** | Param / returned heap Range; user Iterator / coro / dict later | **Landed** ([COI-351](https://linear.app/ardax/issue/COI-351)): numeric free-fn Range params and two-slot Range `CALL`/`RETURN`. Counted `for` without `GetField`. Heap-field / array Range opened as **C2b** rung 1. User `Iterator::next` opened as **C2b** rung 2. Coro / dict stay refuse | `for_in_range_param`; `for_in_range_ret`; checksum; cost gate; flagships flat. No full trait rewrite |
| **C2b** | Heap-field / array Range; then user Iterator / coro / dict | **Rung 2 this PR** ([COI-353](https://linear.app/ardax/issue/COI-353)): user `Iterator::next` on MIR (Q8 two-slot / niche match + B3 CALL; no `BoxValue` in the helper). Counted desugar when `IntoIter` is array / tuple / numeric Range. Rungs 3–4: dict for-in, coro for-in | `for_in_iter`; `for_in_iter_range`; checksum; cost gate; flagships flat |
