...
| `for` / iterator (`for_in_sum`) | **Q6:** counted desugar — array / Vec / `[T; N]` / literal range helpers dense (`sum`, `for_in_range`). **B5:** first-class `let r = 0..n` locals unbox `[start,end]` (`for_in_range_value`). **C2:** free-fn param / returned numeric Range two-slot (`for_in_range_param` / `for_in_range_ret`). **C2b rung 1:** heap-field / array-held Range counted (`for_in_range_field` / `for_in_range_array`). `main` + format / `Vec.push` stays fuse-IL. User `Iterator` / coro / dict for-in still refuse | Ladder (counted + local + param/ret + heap-field range) |
...
| **B5** | Q6 later rungs — first-class range, then user `Iterator` | **Landed** ([COI-343](https://linear.app/ardax/issue/COI-343)): unboxed `let r = 0..n` locals (no `GetField`). Param / returned Range opened as **C2**; heap-field opened as **C2b** rung 1; user `next`, coro, dict stay refuse | `for_in_range_value`; no full trait rewrite |
...
| **C2** | Param / returned heap Range; user Iterator / coro / dict later | **Landed** ([COI-351](https://linear.app/ardax/issue/COI-351)): numeric free-fn Range params and two-slot Range `CALL`/`RETURN`. Counted `for` without `GetField`. Heap-field / array Range opened as **C2b** rung 1. User `Iterator` / coro / dict stay refuse | `for_in_range_param`; `for_in_range_ret`; checksum; cost gate; flagships flat. No full trait rewrite |
| **C2b** | Heap-field / array Range; then user Iterator / coro / dict | **Rung 1 this PR** ([COI-353](https://linear.app/ardax/issue/COI-353)): heap-field and array-held numeric Range counted `for` (`LoadField` 0/1, no `GetField` / `STRING` / `MakeDict` in the helper). Rungs 2–4: user `Iterator::next`, dict for-in, coro for-in | `for_in_range_field`; `for_in_range_array`; checksum; cost gate; flagships flat |
