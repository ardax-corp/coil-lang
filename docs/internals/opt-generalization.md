...
| **C2** | Param / returned heap Range; user Iterator / coro / dict later | **This rung** ([COI-351](https://linear.app/ardax/issue/COI-351)): numeric free-fn Range params and two-slot Range `CALL`/`RETURN`. Counted `for` without `GetField`. User `Iterator` / coro / dict / heap-field dict stay refuse. Do not start C3 here | `for_in_range_param`; `for_in_range_ret`; checksum; cost gate; flagships flat. No full trait rewrite |
