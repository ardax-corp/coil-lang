...
| **B2** | Q7 cost-gate lose on `fib` / `tak` | **Landed** ([COI-340](https://linear.app/ardax/issue/COI-340)): dense emit convoys one-word `CALL` like fuse-IL (stack args / results; Seek only when extras live). Cost gate unchanged. Flagship `fib` / `tak` archives stay identical (reconstruct fuse-selects to the same bytes) | Checksum + embed wall **≤ fuse**. No skip-the-gate. Regular CALL reconstruct, not a tak opcode |
