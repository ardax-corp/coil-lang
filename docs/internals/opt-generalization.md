...
| **C1** | Self two-slot (→ N-slot) `CALL` / `RETURN` | **This rung** ([COI-349](https://linear.app/ardax/issue/COI-349)): drop the self two-slot specialize refuse. Helper / sibling / self share `ret_words` + dest + `dest_hi`. Keep/refuse is the cost gate. **N-slot reach:** infer/lower use `ret_words >= 2` capped at `MAX_MODELED_RET_WORDS` (2). N>2 needs extra dests + `CALL`/`RETURN` archive encoding — not a self-vs-helper wall. Do not start C2 here | `self_two_slot.hy`; `option_self_call`; checksum; cost gate; flagships flat. No new opcode |
