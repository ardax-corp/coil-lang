...
| **C2** | Param / returned heap Range; user Iterator / coro / dict later | **Landed** ([COI-351](https://linear.app/ardax/issue/COI-351)): numeric free-fn Range params and two-slot Range `CALL`/`RETURN`. Counted `for` without `GetField`. Heap-field / array Range opened as **C2b** rung 1. User `Iterator::next` opened as **C2b** rung 2. Dict for-in opened as **C2b** rung 3. Coro stays refuse | `for_in_range_param`; `for_in_range_ret`; checksum; cost gate; flagships flat. No full trait rewrite |
| **C2b** | Heap-field / array Range; then user Iterator / coro / dict | **Rung 3 this PR** ([COI-353](https://linear.app/ardax/issue/COI-353)): dict for-in on MIR (`DictEntries` / `MakeDict` mapped alloc; counted array latch; dense or LIR). Keys/values via entry pairs (`p[0]` / `p[1]`). Counted `into_iter` when `IntoIter` is a dict. Rung 4 leftover: coro for-in. Keep issue open | `for_in_dict`; checksum; cost gate; flagships flat |
