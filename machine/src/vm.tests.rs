use common::{
    ...
    pack_init_typed, pack_set_field_slot,
};
...
fn make_tuple_and_array_keep_declaration_order_and_consume_exactly_arity() {
    for (name, insn) in [
        ...
        let elements: Vec<i64> = match vm.heap().find_object_by_addr(addr) {
            Some(Object::Tuple(gc)) => gc.as_ref().elements.iter().map(Value::as_int).collect(),
            Some(Object::Array(gc)) => gc.as_ref().elements.iter().map(Value::as_int).collect(),
...
fn dense_index(flags: u8, dest: u8, arr: u8, idx: u8) -> Byte {
    let op = ((flags as u32) << 24) | ((dest as u32) << 16) | ((arr as u32) << 8) | (idx as u32);
...
fn nested_enum_gc_traces_correctly() {
    ...
    let (outer_obj, _) = heap.alloc(
        ObjEnum {
            tag: 0,
            payload: crate::EnumPayload::two(Member::Object(inner_obj), Member::Object(string_obj)),
        },
        Object::Enum,
    );
...
fn host_invoke_void_ok_none_pushes_unit() {
    ...
    assert_eq!(
        vm.pop().as_int(),
        42,
        "CONST after HostInvoke must not be lost"
    );
    assert_eq!(vm.pop(), Value::default(), "void Ok(None) must push unit");
}
...
fn load_shared_program_pins_arc_without_copy() {
    ...
    let code = Arc::new(unsafe {
        std::slice::from_raw_parts(archived.as_ptr().cast::<RawByte>(), archived.len()).to_vec()
    });
...
fn userland_library_survives_gc_for_ffi_invoke() {
    ...
    let sig =
        FfiSignature::from_parts("sum", vec![FfiType::Int, FfiType::Int], FfiType::Int).unwrap();
    let fn_id = vm
        .register_ffi_function(lib_val, sig)
...
fn vec_pop_allocates_a_boxed_option() {
    ...
    let value = crate::vec_ops::host_vec_pop(&mut vm.heap, &[Value::from(object.addr())]);
    assert!(vm.heap.find_object_by_addr(value.raw() as u64).is_some());
...
fn static_slot_roots_heap_object_across_gc() {
    ...
    assert!(
        !vm.panicked(),
        "LoadStatic after GC must not see a swept string"
    );
    let _ = vm.restore_output();
    let s = String::from_utf8(take_test_output(buf)).expect("utf-8");
